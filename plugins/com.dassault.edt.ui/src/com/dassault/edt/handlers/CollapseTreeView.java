...
//				Open editor of DRepresentation
				...
				for (DRepresentationElement dRepresentationElement : representationElements) {
					...
					if (dRepresentationElement instanceof DTreeItem dt) {
						collapseDTreeItem(dt);
						collapseNode(dRepresentation, dt);
					}
					EDTLog.toConsole(name + " is collapsed");
				}
	 ...
	 * @param dt : tree node
	 */
	protected void collapseDTreeItem(DTreeItem dt) {
		...
		/* Create Command */
		CompoundCommand collapseDTreeItemCmd = new CompoundCommand(
				MessageFormat.format(Messages.DTreeItemExpansionChangeCommand_collapseItem, dt.getName()));

		/* Add expand command */
		collapseDTreeItemCmd.append(
				new DTreeItemExpansionChangeCommand(globalContext, domain, dt, new NullProgressMonitor(), false));

		/* Refresh command */
		SiriusCommand siriusCommand = new SiriusCommand(domain);
		siriusCommand.getTasks().add(new RefreshTreeElementTask(dt));
		collapseDTreeItemCmd.append(siriusCommand);
